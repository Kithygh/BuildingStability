...
def GetDBs(count = 10, config_file="ssh.config", server_name="odin", keyfile="id_rsa", passphrase_file="passphrase"):
    with paramiko.SSHClient() as ssh:
        folder_name = "/mnt/exiles-DB/" + (datetime.datetime.now() - datetime.timedelta(2)).strftime("%Y-%m-%d") + "_09-09"
        #print(folder_name)
        ...
        files.sort(key=lambda x: x.st_size, reverse=True)
        for progress, file in enumerate(files[:count]):
            print(f"Downloading {file.filename} size:{file.st_size} ({progress+1} / {len(files)})") 
